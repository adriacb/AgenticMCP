...
ASYNC_CONTEXT_MANAGER_ERROR = (
    "As of mcpagent 0.1.0, MultiServerMCPClient cannot be used as a context manager (e.g., async with MultiServerMCPClient(...)). "
...
class MultiServerMCPClient:
    """Client for connecting to multiple MCP servers and loading tools, prompts and resources from them."""
    ...
    def __init__(
        ...
        ```python
        from mcpagent.client import MultiServerMCPClient
        ...
        ```python
        from mcpagent.client import MultiServerMCPClient
        from mcpagent.tools import load_mcp_tools
    ...
    async def get_tools(self, *, server_name: str | None = None) -> list[Any]:
        ...
        for connection in self.connections.values():
            async with create_session(connection) as session:
                load_tool_task = asyncio.create_task(session.get_tools())
